...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#020306] text-zinc-400 selection:bg-indigo-500 selection:text-white">
              ...
              <div className="aspect-[16/9] bg-[#050608] flex items-center justify-center relative overflow-hidden">
                ...
                <div className="flex flex-col items-center gap-6 relative z-10 text-center px-12">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-xl font-bold text-white">AI-powered collaboration</h3>
                    <p className="text-sm text-zinc-500 max-w-sm">Sign up to see the full workspace.</p>
                  </div>
                  <Link to="/auth" className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-all uppercase tracking-widest">
                    Get access →
                  </Link>
                </div>
              </div>
      ...
      <section className="py-48 px-8 border-y border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              { icon: <Lock />, title: "Secure by Design", desc: "All data is encrypted in transit and at rest. Your workspace is private and isolated." },
              { icon: <Globe />, title: "Real-time Sync", desc: "Changes sync instantly across all team members. No refresh needed, no conflicts." },
              { icon: <Cpu />, title: "Contextual AI", desc: "Ask the AI assistant anything about your projects. It understands context and helps you write, plan, and ship." }
            ].map((feature, i) => (
              <motion.div
      ...
      <footer className="py-16 px-8 bg-[#040507] border-t border-white/5">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center text-white">
              <Layout size={18} />
            </div>
            <span className="font-bold text-white tracking-tight">GhostLink</span>
          </div>
          <p className="text-[10px] font-bold text-zinc-700 uppercase tracking-widest">
            © {new Date().getFullYear()} GhostLink. Early Access Beta.
          </p>
          <div className="flex gap-6">
            <Link to="/auth" className="text-[10px] font-bold text-zinc-600 hover:text-zinc-400 transition-colors uppercase tracking-widest">Sign Up</Link>
            <Link to="/auth" className="text-[10px] font-bold text-zinc-600 hover:text-zinc-400 transition-colors uppercase tracking-widest">Log In</Link>
          </div>
        </div>
      </footer>
    </div>
